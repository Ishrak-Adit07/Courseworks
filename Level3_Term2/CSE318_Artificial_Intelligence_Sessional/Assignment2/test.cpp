...
struct Board {
    ...
    void print_board(ofstream& output_file) { // Take output_file as parameter
    ...
    bool is_game_over() {
        bool player1_side_empty = all_of(bins.begin(), bins.begin() + NUMBER_OF_BINS, [](int stones) { return stones == 0; });
        bool player2_side_empty = all_of(bins.begin() + NUMBER_OF_BINS + 1, bins.begin() + TOTAL_BINS - 1, [](int stones) { return stones == 0; });
    ...
    int get_winner() {
        if (bins[PLAYER_1_STORAGE] > bins[PLAYER_2_STORAGE]) return PLAYER_1;
        if (bins[PLAYER_2_STORAGE] > bins[PLAYER_1_STORAGE]) return PLAYER_2;
        return 2; // Tie
    }
};
...
int minimax(Board& board, int depth, int alpha, int beta, int maxPlayer, int heuristic) {
    ...
    if (maxPlayer == PLAYER_1) {
        ...
        for (int i = 0; i < NUMBER_OF_BINS; i++) {
            if (board.bins[i] == 0) continue; // Skip empty bins
        ...
        int minValue = INF;
        for (int i = 0; i < NUMBER_OF_BINS; i++) {
            if (board.bins[NUMBER_OF_BINS + i + 1] == 0) continue; // Skip empty bins
            Board temp_board = board;
...
int main() {
    ofstream output_file("game_states.txt"); // Create output file stream

    int depth = 3; // Set the depth for Minimax algorithm
    int heuristic = 1; // Choose heuristic type
    Board board;

    while (!board.is_game_over()) {
        board.print_board(output_file); // Print to file

        // Player 1's turn
        int player1_move = get_best_move(board, depth, heuristic, PLAYER_1);
        ...
        // Player 2's turn
        board.print_board(output_file); // Print to file
        int player2_move = get_best_move(board, depth, heuristic, PLAYER_2);
    ...
    board.print_board(output_file); // Print final state to file
    output_file << "Winner: Player " << board.get_winner() + 1 << endl;
